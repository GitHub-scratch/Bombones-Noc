...
export default function Guarda({ fetchData, showToast }) {
  ...
  const generateDispatchReport = (data) => {
    const ws = XLSX.utils.json_to_sheet([{
      ...
      Unidad: data.unit,
      Formato: data.packaging,
      Destino: data.destination,
      Empresa: 'Bombones Noc'
    }]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Guía de Despacho");
    XLSX.writeFile(wb, `Despacho_${data.pt_lote}_${new Date().getTime()}.xlsx`);
  };
