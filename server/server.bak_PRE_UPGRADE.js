...
app.post('/api/materials', (req, res) => {
  const { name, unit, min_stock, category } = req.body;
  db.run('INSERT INTO materials (name, unit, min_stock, category) VALUES (?, ?, ?, ?)', [name, unit, min_stock || 0, category || 'OTRO'], function(err) {
    if (err) res.status(500).json({ error: err.message });
    else res.json({ id: this.lastID, name, unit, min_stock, category: category || 'OTRO' });
  });
});
...
app.put('/api/materials/:id', (req, res) => {
  ...
  db.run('UPDATE materials SET name = ?, unit = ?, min_stock = ?, category = ? WHERE id = ?',
    [name, unit, parseFloat(min_stock) || 0, category || 'OTRO', id], function(err) {
